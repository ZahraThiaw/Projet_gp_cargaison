...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // const GetDatauser = async (): Promise<IUser[]> => {
  //   const response = await fetch("../php/login.php");
  //   const data = await response.json();
  //   return data.cargo;
  // }

  // const duser = await GetDatauser();
  //   console.log(duser);
  ...
  function validateForm(formulaire: HTMLFormElement): boolean {
    let isValid = true;
    ...
    inputs.forEach(input => {
      ...
      if (inputElement.value.trim() === "") {
        ...
        if ((inputElement.id === "client-email" && !isValidEmail(inputElement.value)) ||(inputElement.id === "recipient-email" && !isValidEmail(inputElement.value)) ||(inputElement.id === "email-user" && !isValidEmail(inputElement.value))) {
  ...
  const userpage = document.getElementById('user-page') as HTMLDivElement;
  const main_gestionnaire = document.getElementById('main_gestionnaire') as HTMLDivElement;
